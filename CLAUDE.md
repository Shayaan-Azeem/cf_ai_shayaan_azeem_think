...
Think is a cross-platform markdown note-taking app for macOS and Windows, built with Tauri v2 (Rust backend) + React/TypeScript/Tailwind (frontend) + TipTap (WYSIWYG editor) + Tantivy (full-text search).
...
3. Submit for notarization:
   ```bash
   xcrun notarytool submit src-tauri/target/release/bundle/macos/Think.zip \
     --apple-id "$APPLE_ID" --password "$APPLE_PASSWORD" --team-id "$APPLE_TEAM_ID" --wait
   ```

4. Staple the notarization ticket:
   ```bash
   xcrun stapler staple src-tauri/target/release/bundle/macos/Think.app
   ```

5. Create DMG with Applications symlink:
   ```bash
   mkdir -p /tmp/think-dmg
   cp -R src-tauri/target/release/bundle/macos/Think.app /tmp/think-dmg/
   ln -sf /Applications /tmp/think-dmg/Applications
   hdiutil create -volname "Think" -srcfolder /tmp/think-dmg -ov -format UDZO \
     src-tauri/target/release/bundle/dmg/Think_VERSION_universal.dmg
   rm -rf /tmp/think-dmg
   ```
...
7. The build also generates an updater manifest at:
   ```
   src-tauri/target/release/bundle/macos/Think.app.tar.gz.sig
   src-tauri/target/release/bundle/macos/Think.app.tar.gz
   ```
...
3. Outputs will be in `src-tauri/target/release/bundle/`:
   - `msi/Think_VERSION_x64_en-US.msi` - MSI installer
   - `nsis/Think_VERSION_x64-setup.exe` - NSIS setup (recommended for distribution)

4. Get checksum and upload:
   ```powershell
   Get-FileHash .\src-tauri\target\release\bundle\nsis\Think_VERSION_x64-setup.exe -Algorithm SHA256
   ```
...
- On startup (after 3s delay) and manually via Settings → General → "Check for Updates", the app fetches:
  `https://github.com/erictli/think/releases/latest/download/latest.json`
...
```json
{
  ...
  "platforms": {
    "darwin-universal": {
      ...
      "url": "https://github.com/erictli/think/releases/download/vVERSION/Think.app.tar.gz"
    },
    "windows-x86_64": {
      "signature": "CONTENTS_OF .nsis.zip.sig FILE",
      "url": "https://github.com/erictli/think/releases/download/vVERSION/Think_VERSION_x64-setup.nsis.zip"
    }
  }
}
```
...
2. Upload these assets:
   ...
   - macOS: `Think_VERSION_universal.dmg` (for manual download) and `Think.app.tar.gz` (for auto-update)
   - Windows: `Think_VERSION_x64-setup.exe` (for manual download) and the NSIS `.zip` (for auto-update)
