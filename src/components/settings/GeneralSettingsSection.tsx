...
export function GeneralSettingsSection() {
  ...
  const handleChangeFolder = async () => {
    try {
      ...
      if (selected) {
        ...
        // Reload theme/font settings from the new folder's .think/settings.json
  ...
  return (
    <div className="space-y-8">
      ...
      {/* About */}
      <section>
        <h2 className="text-xl font-medium mb-0.5">About Think</h2>
        <p className="text-sm text-text-muted mb-3">
          You are currently using Think v{appVersion || "..."}. Learn more on{" "}
          <a
            href="https://github.com/erictli/think"
            target="_blank"
            rel="noopener noreferrer"
            className="text-text-muted border-b border-text-muted/50 hover:text-text hover:border-text cursor-pointer transition-colors"
          >
