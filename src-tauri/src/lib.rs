...
// Per-folder settings (stored in .think/settings.json within notes folder)
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct Settings {
...
pub struct AppState {
    ...
    pub settings: RwLock<Settings>,      // per-folder settings (stored in .think/)
...
/// Filter for WalkDir: skips dot-directories (e.g. .think, .git) and assets/.
...
fn id_from_abs_path(notes_root: &Path, file_path: &Path) -> Option<String> {
    ...
    // Skip excluded directories (dot-dirs catch .think, .git, etc.)
...
// Get per-folder settings file path (in .think/ within notes folder)
fn get_settings_path(notes_folder: &str) -> PathBuf {
    let think_dir = PathBuf::from(notes_folder).join(".think");
    std::fs::create_dir_all(&think_dir).ok();
    think_dir.join("settings.json")
}
...
fn set_notes_folder(app: AppHandle, path: String, state: State<AppState>) -> Result<(), String> {
    ...
    // Create .think config folder
    let think_dir = path_buf.join(".think");
    std::fs::create_dir_all(&think_dir).map_err(|e| e.to_string())?;

    // Verify write access early to avoid later silent failures
    let write_test_path = think_dir.join(".write-test");
